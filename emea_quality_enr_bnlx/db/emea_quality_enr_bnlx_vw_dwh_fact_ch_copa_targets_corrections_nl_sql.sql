CREATE VIEW emea_quality_enr_bnlx.vw_dwh_fact_ch_copa_targets_corrections_nl_sql (
  `0accnt_asgn_1`,
  `0billtoprty`,
  `0bill_type`,
  `0calmonth`,
...
AS SELECT 
  ...
  sum(targets_gross_sales_target_calc) as targets_gross_sales_target_calc,
  sum(targets_discount_target_calc) as targets_discount_target_calc,
  sum(targets_net_sales_target_calc) as targets_net_sales_target_calc,
  customer_country_code,
  customer_cluster,
  customer_channel,
  customer_organization,
  ...
  date_text,
    CASE 
        WHEN YEAR(COPA_DATE) = YEAR(CURRENT_DATE) AND COPA_DATE < CURRENT_DATE
        THEN 2 
        WHEN YEAR(COPA_DATE) =YEAR(CURRENT_DATE - INTERVAL '1 year')  AND COPA_DATE < (CURRENT_DATE - INTERVAL '1 year')
        THEN 1
        ELSE 0 
    END AS period_ytd_pytd,
   CASE 
        WHEN YEAR(COPA_DATE) = YEAR(CURRENT_DATE) AND COPA_DATE <= LAST_DAY(CURRENT_DATE - INTERVAL '1 month')
        THEN 2 
        WHEN YEAR(COPA_DATE) = (YEAR(CURRENT_DATE)  -1) AND COPA_DATE <= LAST_DAY((CURRENT_DATE - INTERVAL '1 year') - INTERVAL '1 month')
        THEN 1
        ELSE 0 
    END as period_ytd_pytd_full_month,
  CASE 
        WHEN concat(year(COPA_DATE),month(COPA_DATE))= concat(year(CURRENT_DATE),month(CURRENT_DATE)) AND COPA_DATE < CURRENT_DATE
        THEN 2
        WHEN concat(year(COPA_DATE),month(COPA_DATE))= concat(year(CURRENT_DATE - INTERVAL '1 year'),month(CURRENT_DATE)) AND COPA_DATE < CURRENT_DATE - INTERVAL '1 year'
        THEN 1
        ELSE 0
    END AS period_mtd_pmtd,
  CASE 
        WHEN YEAR(COPA_DATE) = (YEAR(CURRENT_DATE)  -1)
        THEN 2
        WHEN YEAR(COPA_DATE)  = (YEAR(CURRENT_DATE)  -2)
        THEN 1
        ELSE 0
...
,IFNULL(a.targets_gross_sales_target,0) as targets_gross_sales_target_calc
,IFNULL(a.targets_discount_target,0) as targets_discount_target_calc
,IFNULL(a.targets_net_sales_target,0) as targets_net_sales_target_calc


,ce.customer_country_code
,ce.customer_cluster
,ce.customer_channel
,ce.customer_organization
,a.targets_objective_type
...
,pe.0sapmaterial_code_trim
  ...
  END AS COPA_DATE,
TO_DATE(CONCAT(0CALMONTH, '01'), 'yyyyMMdd') AS COPA_YEAR_MONTH,
  TO_DATE(CONCAT(0CALYEAR, '0101'), 'yyyyMMdd') AS COPA_YEAR,
  CASE
    WHEN 0CALDAY = '00000000' then CONCAT(0CALMONTH, '01')
    ELSE 0CALDAY
  END AS DATE_TEXT,
    a.RLS_AUTH_SID
FROM emea_quality_enr_bnlx.fact_prep_sap_ch_target_corrections_nl a 
...
LEFT JOIN emea_quality_enr_bnlx.dim_prepare_ch_customer_enrichment_dimension_nl ce
 on ce.0cust_sales_combined=a.0cust_sales_trim
 ...
 LEFT JOIN emea_quality_enr_bnlx.dim_prepare_ch_product_enrichment_dimension_nl pe on pe.0sapmaterial_code_trim=a.0material_trim
