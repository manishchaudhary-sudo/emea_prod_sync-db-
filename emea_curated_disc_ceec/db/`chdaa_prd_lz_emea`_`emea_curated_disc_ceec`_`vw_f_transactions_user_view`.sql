CREATE MATERIALIZED VIEW `chdaa_prd_lz_emea`.`emea_curated_disc_ceec`.`vw_f_transactions_user_view` (
...
CAST(Trans.exfactory_price AS DECIMAL(18, 2)) as ExFactory_price,
...
,Trans.ETLLastModifiedDate

FROM emea_curated_disc_ceec.fact_transactions  Trans
left join emea_curated_disc_ceec.dim_customers cust on cust.customer_id=trans.transaction_customer_id
left join emea_curated_disc_ceec.dim_products prod on prod.product_id=trans.transaction_product_id 
left join emea_curated_disc_ceec.dim_customers cust1 on cust1.customer_bk=trans.transaction_customer_bk 
and cust1.customer_is_valid=true 
left join  emea_curated_disc_ceec.dim_date time1 on cast(time1.time_id as date)=cast(trans.transaction_time_id as date)
...
Trans.KPI,Trans.ETLLastModifiedDate,
Trans.Kamsoft_program_name,
Trans.Kamsoft_program_ID,
Trans.Date_purchase,
Trans.Expiry_date,
