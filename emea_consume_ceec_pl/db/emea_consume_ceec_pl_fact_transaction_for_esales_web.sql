CREATE VIEW emea_consume_ceec_pl.fact_transaction_for_esales_web (
  year,
  month,
  transaction_source,
  data_delivery_date,
...
select 
    ...
    transaction_customer_bk id_external_reporting,ETLLastModifiedDate,
...
where 
    ...
    and transaction_time_time_id >= concat(year(current_date)-1, '-01-01')
group by 
    year(transaction_time_time_id), 
    month(transaction_time_time_id), 
    'n/a', 
    data_delivery_date,
    kpi,ETLLastModifiedDate,
...
select 
    ...
    kpi,
    BLOZ,
    ID_external_reporting,ETLLastModifiedDate,
...
from 
    (select 
        ...
        Estimated_order_value_exfactory_price,
        row_number() over (
            partition by transaction_source, kpi, year(transaction_time_time_id), month(transaction_time_time_id), transaction_product_bk, transaction_customer_bk 
            order by transaction_time_time_id desc, data_delivery_date desc, volume desc 
        ) rw_num
    from emea_curated_disc_ceec.vw_f_transactions_user_view
    where 
        kpi in ('Stock') 
