CREATE VIEW emea_quality_int_bnlx.vw_dim_prepare_ch_customer_enrichment_dimension_be (
...
WITH SCHEMA BINDING
AS (
Select country_code AS customer_country_code,status AS customer_status,cluster AS customer_cluster,channel AS customer_channel,organization AS customer_organization,employee_cwid AS customer_employee_cwid,employee_name AS customer_employee_name,main_sap_customer_code AS customer_main_sap_customer_code_1,sap_customer_code AS customer_sap_customer_code,sap_customer_name AS customer_sap_customer_name,sap_customer_zip_code AS customer_sap_customer_zip_code,sap_customer_city AS customer_sap_customer_city,sap_customer_country AS customer_sap_customer_country,sap_apb_code AS customer_sap_apb_code,sap_customer_code_trim,0cust_sales,0cust_sales___t,0distr_chan,0distr_chan___t,0division,0division___t,0salesorg,0salesorg___t,0cust_group,0cust_group___t,0cust_grp1,0cust_grp2,0cust_grp3,0cust_grp4,0cust_grp5,dcustgrp1,dcustgrp1___t,dcustgrp2,dcustgrp2___t,dcustgrp3,dcustgrp3___t,dcustgrp4,dcustgrp4___t,dcustgrp5,dcustgrp5___t,dctyspcod,dctyspcod___t,coalesce(0cust_sales_trim_sap, sap_customer_code_trim) AS 0cust_sales_combined,
coalesce(0distr_chan, '00') AS 0distr_chan_comb,
...
(
 ...
 FROM
    (
      ...
      Select  0cust_sales,0cust_sales___t,0distr_chan,0distr_chan___t,0division,0division___t,0salesorg,0salesorg___t,  0cust_group,0cust_group___t,0cust_grp1,0cust_grp2,0cust_grp3,0cust_grp4,0cust_grp5,dcustgrp1,  dcustgrp1___t,dcustgrp2,dcustgrp2___t,dcustgrp3,dcustgrp3___t,dcustgrp4,dcustgrp4___t,dcustgrp5, dcustgrp5___t,dctyspcod,dctyspcod___t,0cust_sales_trim_sap,0price_grp from emea_quality_int_bnlx.dim_custsales_p_001   where (0salesorg = 'BBHC' OR 0salesorg = '' )
    ) custs
    full outer join
    (
      
      Select d_customer_mapping_code,country_code,status,cluster,channel,organization,employee_cwid,employee_name,  main_sap_customer_code,sap_customer_code,sap_customer_name,sap_customer_zip_code,sap_customer_city,sap_customer_country, sap_apb_code,update_date,update_by,extract_date,extract_by
      from emea_quality_int_bnlx.man_ch_customer_mapping where country_code='BE'
